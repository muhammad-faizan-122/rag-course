...
def process_and_print_query(
    query, correct_label, response_std, tokens_std, response_simp, tokens_simp
):
    ...
    print(
        f"  Simplified  → Label: {label_simp_colored} | Tokens: {tokens_simp_colored}\n"
    )
...
class ChatBot:
    ...
    def chat(
        self, prompt: str, role: str = "user", return_stats=False
    ) -> Dict[str, str]:
        """
        Handles a single round of user interaction and updates the conversation context.
        """
        with self.tracer.start_as_current_span(
            "agent_call", openinference_span_kind="agent"
        ) as span:
            ...
            params_dict, total_tokens = self.generator_function(
                prompt
...
def parse_json_output(llm_output):
    ...
    try:
        ...
        llm_output = (
            ...
            .replace("}}", "}")
            .replace("{{", "{")
        )  # Remove any erroneous structures
...
def generate_metadata_from_query(query):
    ...
    response = generate_with_single_input(
        PROMPT, temperature=0, max_tokens=1500
    )  # @REPLACE EQUALS None
